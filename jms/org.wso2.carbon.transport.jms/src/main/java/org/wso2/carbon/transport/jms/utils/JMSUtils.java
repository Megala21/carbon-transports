...
public class JMSUtils {
                    ...
                    if (null != context.getEnvironment().get(JMSConstants.NAMING_FACTORY_INITIAL)) {
                        initialContextProperties.put(JMSConstants.NAMING_FACTORY_INITIAL,
                                                     context.getEnvironment()
                                                            .get(JMSConstants.NAMING_FACTORY_INITIAL));
                    }
        ...
        try {
            if (message instanceof TextMessage) {
                ...
                while (mapKeys.hasMoreElements()) {
                    ...
                    if (null != mapValue) {
                        mapCarbonMessage.setString(mapKey, mapValue);
                    } else {
                        mapCarbonMessage.setByte(mapKey, mapMessage.getBytes(mapKey));
                    }
                }
                jmsCarbonMessage = mapCarbonMessage;
            } else {
                jmsCarbonMessage = new TextCarbonMessage("jms");
            }
    ...
    public static void setTransportHeaders(Message message, Map<String, Object> headerMap)
        ...
        try {
            ...
            throw new JMSConnectorException("Error while setting transport headers", exception);
        }
    }
}
